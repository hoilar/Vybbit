...
class CreateRoomPayload(BaseModel):
    ...
    game_mode: str = Field(default="chill", pattern="^(chill|exposed)$")
...
class ExposedSpotifyPayload(BaseModel):
    spotify_id: str = Field(min_length=1)
    display_name: str = Field(min_length=1, max_length=64)
    tracks: list[dict[str, Any]] = Field(default_factory=list)
    category: str = Field(default="top10")
...
class Room:
    ...
    game_mode: str = "chill"  # "chill" or "exposed"
    round_category: Optional[str] = None  # "top10", "nostalgia", "guilty_pleasure", "new_discovery"
...
class RoomManager:
    ...
    async def create_room(self, host_name: str, guess_duration_seconds: int, game_mode: str = "chill") -> tuple[Room, Player]:
    ...
    async def start_game(self, code: str, player_id: str, rounds: int) -> Room:
        async with self.lock:
            ...
            if any(not player.ready(room.game_mode) for player in room.players.values()):
            ...
            if room.game_mode == "exposed" and room.round_category:
                for track in track_pool:
                    track["category"] = room.round_category
            round_pool = [
                track
                for track in track_pool
    ...
    def serialize_room(self, room: Room) -> dict[str, Any]:
        ...
        if current_round:
            round_payload = {
                ...
                "category": current_round.get("category") if room.reveal_round else None,
            }
        return {
            "code": room.code,
            "gameMode": room.game_mode,
            "roundCategory": room.round_category,
    ...
    async def sync_exposed_player_spotify(self, code: str, player_id: str, payload: ExposedSpotifyPayload) -> Room:
        async with self.lock:
            room = self._get_room(code)
            player = self._get_player(room, player_id)
            if room.game_mode != "exposed":
                raise HTTPException(status_code=400, detail="Kun tilgjengelig i Exposed-modus.")
            player.spotify_id = payload.spotify_id
            player.spotify_name = payload.display_name
            player.tracks = payload.tracks[:300]
            player.guest_playlist_synced = True
    ...
    async def set_round_category(self, code: str, player_id: str, category: str) -> Room:
        async with self.lock:
            room = self._get_room(code)
            self._assert_host(room, player_id)
            if room.game_mode != "exposed":
                raise HTTPException(status_code=400, detail="Kun tilgjengelig i Exposed-modus.")
            if room.started:
                raise HTTPException(status_code=400, detail="Kan ikke endre kategori mens spillet paar.")
            room.round_category = category
            return room
...
async def create_room(payload: CreateRoomPayload) -> dict[str, Any]:
    room, host = await manager.create_room(payload.host_name, 15, payload.game_mode)
...
@app.post("/api/rooms/{code}/players/{player_id}/exposed-spotify")
async def sync_exposed_player_spotify(code: str, player_id: str, payload: ExposedSpotifyPayload) -> dict[str, Any]:
    room = await manager.sync_exposed_player_spotify(code, player_id, payload)
    await manager.broadcast_state(room.code)
    return manager.serialize_room(room)


@app.post("/api/rooms/{code}/players/{player_id}/category")
async def set_round_category(code: str, player_id: str, payload: SetCategoryPayload) -> dict[str, Any]:
    room = await manager.set_round_category(code, player_id, payload.category)
    await manager.broadcast_state(room.code)
    return manager.serialize_room(room)
